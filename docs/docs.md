...
**环境要求：**
- Windows 操作系统
- Windows wx PC 版（`4.1.9` ~ `4.1.13.7` 版本）
...
### 此功能开发中，有需要可前往[交流页面](https://wxbot.siverking.online/docs.html?c=%E4%BA%A4%E6%B5%81)联系作者
...
连接成功后，扫码账号就是授权账号。可以在微信 ClawBot 会话中直接发送自然语言与内置 Agent 对话，也可以使用 `/agent`、`/agent会话`、`/agent新会话`、`/agent停止` 等指令。未主动断开时，连接凭据会保存在本机，面板重启后会自动尝试恢复。
...
4. 关闭wx自动更新。
5. 适配 Windows wx `4.1.9` - `4.1.13.7`。
