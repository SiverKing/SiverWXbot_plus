...
- [🤖 Siver WX机器人 (wxbot\_plus)](#-siver-wx机器人-wxbot_plus)
  - [目录](#目录)
  - [📦 安装部署](#-安装部署)
    - [方法一：直接使用可执行程序 exe（推荐新手）](#方法一直接使用可执行程序-exe推荐新手)
    - [方法二：下载源码使用 Python 部署](#方法二下载源码使用-python-部署)
  - [🖥️ 管理面板](#️-管理面板)
    - [面板账密](#面板账密)
    - [模型配置](#模型配置)
    - [Prompt 配置](#prompt-配置)
...
5. **填写模型 ID：** DusAPI 的模型根据你的 API 密钥分组填写，分组为 gpt 就填 gpt 类型的模型，为 claude 就填 claude 的模型。
   ...
   - 若选择其他兼容接口，参照底部[注意事项](#注意事项)填写模型内容。
...
3. 大概等待十几秒后程序启动完成，可以将面板弹窗点掉，查看面板下方日志是否启动成功。

4. 在**手机 wx** 上给 `文件传输助手` 发送以下指令进行测试：
   - 发送 `/状态` → 查看是否运行成功
   - 发送 `/接口测试****`（将 `****` 替换成你要测试的话，如 `/接口测试你好`）→ 测试接口配置是否成功

5. 若配置和启动都没问题，将会触发自动回复。发送 `/指令` 可查看更多指令（具体指令说明见下方[消息指令调用参考](#消息指令调用参考)）。测试成功可在状态面板查看当前运行状态。

6. 测试成功后，点击面板上的 `关闭机器人` 按钮，随后几秒后所有功能将被关闭。
...
1. 启动机器人时不得有微信小窗口（只保留主窗口），同时确保主窗口没有最小化。
2. 电脑自动休眠、黑屏都设为**从不**。
3. 如需离线邮件提醒，请在配置面板修改设置邮件。
4. 关闭微信自动更新。
5. 适配 Windows wx `4.1.7` - `4.1.8.24`。
6. 程序接管微信运行时，请勿手动干预，避免影响自动化操作。
7. **各 SDK 接口填写说明：**
   - **DusAPI**：填写 Key 和模型 ID，模型 ID 可从 https://docs.dusapi.com 查看
   - **OpenAI SDK**：填写 Key、URL、模型1、模型2
   - **Dify**：填写 Key、URL，模型1/2 填写 Dify 的工作流 ID
   - **Coze**：仅需填写 Key，模型1/2 填写 Coze 智能体 bot_id，无需填写 URL
